...
All notable changes to IB-Tool 3 will be documented in this file.
...
### Added
- **Per-file validation-skip granularity** (`helpers/check.py`, `ibtool.py`): `InputValidator.validate_all()` now accepts `file_checksums`/`previous_cache` and returns a `ValidationResult.layer_cache`. Previously, *Check* skipped re-validation only when the entire input (all 5 files, CRS, and every parameter) was unchanged since the last run — fixing a single unrelated issue (e.g. the target CRS) forced every file to be fully re-scanned again. Now the expensive per-file content checks (geometry validity, ATKIS/field checks, multipart-line checks) are cached per file and reused whenever that file's MD5 checksum is unchanged, while cheap checks that can legitimately change independently of file content (CRS match, feature counts, part:HU ratio, UI parameters) are always re-evaluated. `IBTool` threads its existing `_file_checksums` through `run_validation()` and keeps the returned cache in a new `_validation_layer_cache` attribute (cleared by "Reset Config", not by ordinary path/parameter edits).
- **Release ZIP guard and import smoke test** (`scripts/create_release_zip.py`): `run_guard()` fails the build on forbidden file extensions (`.exe .dll .so .dylib .sh .bat .cmd`), a set executable bit, unexpected binary files (only `icon.png` and `*.qm` translations are allowed), or a ZIP over the 25 MB plugins.qgis.org limit. `run_import_smoke_test()` extracts the built ZIP and statically resolves every local `from ibtool...`/relative import against the files on disk, guarding against a repeat of the 0.2.2 `helpers/debug_utils.py` incident.

### Changed
- **Release ZIP packaging switched from an exclusion list to an explicit whitelist** (`scripts/create_release_zip.py`): `WHITELIST_FILES`/`WHITELIST_DIRS` now name exactly what ships (`__init__.py`, `metadata.txt`, `icon.png`, `LICENSE`, `README.md`, `requirements.txt`, `resources.qrc`, plus the `helpers/`, `i18n/`, `ibtool/`, `ibtool_tools/` directories), instead of trying to exclude everything that shouldn't. This also drops `coverage.xml`, `IB-Tool_Icon_2HiRes.png` (an unreferenced 1.2 MB icon draft), and `requirements-dev.txt` from the release ZIP, none of which were ever needed at runtime.
- **Release ZIP folder and filename are now the constant `ibtool`** (`PLUGIN_FOLDER = "ibtool"` in `scripts/create_release_zip.py`), independent of the checkout directory name — previously the internal folder (and, transitively, the filename) was derived from `repo_root.name`, so it only happened to read `IB-Tool-3` because that is this repository's checkout name. The release ZIP is now `dist/ibtool.zip`. Manual git-clone installs still keep the repository's hyphenated folder name, but no longer need a manual rename either — `__init__.py`'s existing virtual `ibtool` package registration and QGIS's own `__import__()`-based plugin loader both already handle a hyphenated folder name correctly. Removed the now-obsolete manual-rename instructions from `README.md` and `docs/plugin-architecture.md`.
- **`metadata.txt` shortened and clarified for the plugins.qgis.org submission**: `description` is now a single sentence (it previously duplicated `about`); `about` now documents the scipy/networkx runtime dependency with OSGeo4W install instructions and the German-ATKIS scope of the expected input data; added `qgisMaximumVersion=3.50` to match the documented supported range; aligned the `README.md` license wording with the `GPL-2.0-or-later` license already declared in `metadata.txt`.

### Fixed
- **"Check" always rejected Global Footprint Density on non-English (e.g. German) systems**: `GlobalFootprintDensityBox` (a `QDoubleSpinBox` since the fix below) formats its `.text()` using the widget's locale, which defaults to the OS locale. On German Windows that locale uses `,` as the decimal separator (e.g. `19,0693693015286`), but `_collect_params()` / `helpers/check.py._check_numeric_params()` parse that text with Python's `float()`, which only accepts `.`. Every *Check* click then failed with "'…' ist keine gültige Zahl" regardless of the actual value. Fixed by forcing the box's own locale to `QLocale.c()` (fixed dot-decimal) in `IBToolDialog.__init__()`, independent of the user's system locale.
- **Global Footprint Density could only be entered as a whole percentage**: `GlobalFootprintDensityBox` on the Parameter step was a `QSpinBox`, silently rounding any fractional input even though the underlying `global_footprint_density` parameter has always been a `float` throughout config loading, validation, and processing (see `docs/parameterization.md`). Changed the widget to a `QDoubleSpinBox` with 13 decimal places (range unchanged at 0–100%), and fixed `IBTool.load_config_to_ui()` which was re-truncating the loaded value back to `int()` before displaying it.
...
### Docs
- Removed the manual plugin-folder-rename instructions from `README.md` (Option 1 and Option 2 installation) and rewrote `docs/plugin-architecture.md`'s "Plugin Folder Naming" section to describe the constant `ibtool/` release folder and the existing virtual-package/`__import__()` handling for manual installs.
- Fixed `README.md`'s claim that IB-Tool 3 detects QGIS via the `QGIS_PREFIX_PATH` environment variable at runtime — that variable is only used by the standalone `test/` harness to bootstrap `QgsApplication` outside of QGIS; the plugin itself never reads it while running inside QGIS.
- Updated all remaining `IB-Tool-3.zip` / `IB-Tool-3/` release-asset references in `README.md`, `docs/quickstart.md`, `docs/contributing.md`, and `ai/core/release-conventions.md` to `ibtool.zip` / `ibtool/`.
...
- **GapClose `HOLE_DETECTION_THRESHOLD_M2`** (`GapClose.py`): reduced from `1_000_000` (1 km²) to `10_000` (1 ha). The previous 1 km² cap was far larger than any realistic interior hole and offered no practical protection; 1 ha matches the `max_hole_size` default and prevents accidental fill of legitimate large holes.
- **GapClose debug layer names** (`GapClose.py`): numeric step prefixes removed from all `save_debug_layer` step names (e.g. `"01_hole_closed"` → `"hole_closed"`, `"13.1_final_gap1_70pct"` → `"final_gap1_70pct"`). Debug layers now sort by creation time within a run rather than by an arbitrary hardcoded prefix.
- **PatchRemove debug folder** (`PatchRemove.py`): `_DEBUG_TOOL_NAME` changed from `"07_PatchRemove"` to `"08_PatchRemove"` to reflect that ErodeEmptyAreas now occupies the step-06 slot (and a future step-07 is reserved).
- **`save_debug_layer` path contract** (`helpers/debug_utils.py`): the function no longer appends a `"debug/"` sub-folder automatically. `debug_dir` is now `os.path.join(workspace_path, tool_name)` instead of `os.path.join(workspace_path, "debug", tool_name)`. Callers are responsible for including any desired sub-structure in `workspace_path` (e.g. `base/debug/PART_7`). This enables the new per-partition workspace pattern without double-nesting.
- **EdgeCatch log level on processing errors** (`helpers/edge_catch_utils.py`): when `error_count > 0` in `create_shortest_lines_to_roads`, the combined message is now logged at `WARNING` instead of `INFO`, making processing errors visible even at default log-level filters.
- **Version** (`metadata.txt`): bumped from `0.1.5` to `0.1.6`.
...
### Fixed
- **IB-Tool 3**: Final output was written from `merge` (the pre-GapFix layer) instead of `gap_fixed`, silently discarding all GapFix results; fixed by passing `gap_fixed` to `save_temp_layer_to_gpkg`.
...
### Fixed
- **IB-Tool 3**: `debug_mode` was never forwarded to `gap_fix()` in `ibtool.py`, so no debug files were ever created despite the checkbox being active.
...
- **IB-Tool 3**: Added `_apply_config_to_ui()` method that reads `CONFIG.ini` and populates all dialog fields when `auto_load_last_used = True`.
- **IB-Tool 3**: Added `_save_config_from_ui()` method that writes the current UI state back to `CONFIG.ini`.
