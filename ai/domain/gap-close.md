...
`ibtool_tools/GapClose.py` is called as step 8 in the main pipeline. It
...
settlement (holes already closed)
    ...
    │
    ├── _gap_select(threshold=70%)         → size-limited gaps
    ├── _gap_select(threshold=90%)         → large fully-enclosed gaps
    │
    ├── area < max_gap_size                → size cap on 70%-selection
    │
    ├── merge(all three) + deleteduplicates + native:dissolve
    │
...
### 3. Two overlap thresholds in Process 2

| Filter | Threshold | Purpose |
|--------|-----------|---------|
| `final_gap1` | 70% | Captures normally enclosed gaps, also size-limited |
| `final_gap2` | 90% | Captures large gaps that are almost fully enclosed |

Both are merged before the final dissolve. The 90% filter independently
catches gaps that exceed `max_gap_size` but are geometrically almost
completely surrounded by settlement — a gap that looks "closed" to the eye
but was rejected by the size cap.
...
| `HOLE_DETECTION_THRESHOLD_M2` | `1_000_000` | Fill all holes up to 1 km² to expose hole polygons |
...
- `docs/how-it-works.md` — user-facing pipeline overview (Step 8)
