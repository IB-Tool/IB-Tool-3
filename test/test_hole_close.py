...
class TestShpArea2:
    ...
    def _layer_withmake_square_geom(self, size: float = 100.0) -> QgsVectorLayer:
        layer = make_polygon_layer()
        add_feature_to_layer(layer, make_square_geom(0, 0, size))
        return layer

    @pytest.mark.unit
    def test_area_field_is_created_when_absent(self):
        """shp_area2 adds an 'Area' field when it does not yet exist."""
        layer = self._layer_withmake_square_geom()
    ...
    def test_area_values_match_geometry_area(self):
        ...
        layer = self._layer_withmake_square_geom(100.0)
        shp_area2(layer)
    ...
    def test_multiple_features_all_get_area(self):
        ...
        layer = make_polygon_layer()
        for i in range(3):
            add_feature_to_layer(layer, make_square_geom(float(i * 200), 0.0, 100.0))
        shp_area2(layer)
        for feat in layer.getFeatures():
...
class TestGetHolePolygons:
    ...
    def test_result_is_valid_qgsvectorlayer(self):
        ...
        layer1 = make_polygon_layer()
        add_feature_to_layer(layer1, make_square_geom(0, 0, 10))
        layer2 = make_polygon_layer()
        add_feature_to_layer(layer2, make_square_geom(0, 0, 100))
    ...
    def test_polygon_inside_reference_is_not_a_hole(self):
        ...
        layer1 = make_polygon_layer()
        add_feature_to_layer(layer1, make_square_geom(10, 10, 20))   # 20×20 inside 100×100
        layer2 = make_polygon_layer()
        add_feature_to_layer(layer2, make_square_geom(0, 0, 100))    # 100×100 reference
        result = get_hole_polygons(layer1, layer2)
    ...
    def test_polygon_outside_reference_is_a_hole(self):
        ...
        layer1 = make_polygon_layer()
        add_feature_to_layer(layer1, make_square_geom(200, 200, 20))  # far outside reference
        layer2 = make_polygon_layer()
        add_feature_to_layer(layer2, make_square_geom(0, 0, 100))     # 100×100 reference
        result = get_hole_polygons(layer1, layer2)
    ...
    def test_mixed_features_only_outside_returned(self):
        ...
        layer1 = make_polygon_layer()
        add_feature_to_layer(layer1, make_square_geom(10, 10, 20))   # inside reference → not a hole
        add_feature_to_layer(layer1, make_square_geom(200, 200, 20)) # outside reference → hole
        layer2 = make_polygon_layer()
        add_feature_to_layer(layer2, make_square_geom(0, 0, 100))
        result = get_hole_polygons(layer1, layer2)
        assert result.featureCount() == 1
    ...
    def test_empty_layer1_returns_empty_layer(self):
        ...
        layer1 = make_polygon_layer()          # empty
        layer2 = make_polygon_layer()
        add_feature_to_layer(layer2, make_square_geom(0, 0, 100))
        result = get_hole_polygons(layer1, layer2)
        assert result.featureCount() == 0
    ...
    def test_empty_layer2_returns_all_features(self):
        ...
        layer1 = make_polygon_layer()
        add_feature_to_layer(layer1, make_square_geom(0, 0, 50))
        add_feature_to_layer(layer1, make_square_geom(100, 100, 50))
        layer2 = make_polygon_layer()          # empty reference
        result = get_hole_polygons(layer1, layer2)
    ...
    def test_result_geometry_is_valid(self):
        ...
        layer1 = make_polygon_layer()
        add_feature_to_layer(layer1, make_square_geom(200, 200, 30))  # outside → will be returned
        layer2 = make_polygon_layer()
        add_feature_to_layer(layer2, make_square_geom(0, 0, 100))
        result = get_hole_polygons(layer1, layer2)
...
class TestHoleClose:
    ...
    def test_result_geometries_are_geos_valid(self):
        ...
        layer = make_polygon_layer()
        add_feature_to_layer(layer, _square_with_hole(outer=100, hx=20, hy=20, hole=20))
        result = hole_close(layer, max_hole_size=500)
        for feat in result.getFeatures():
            geom = feat.geometry()
    ...
    def test_large_hole_is_not_filled(self):
        ...
        """
        layer = make_polygon_layer()
        add_feature_to_layer(layer, _square_with_hole(outer=100, hx=30, hy=30, hole=40))
